...
ROOT="${data_path_dir}/da_dataset/NeurIPS_1_filter"
test_case_path="${data_path_dir}/da_dataset/NeurIPS_1_filter/task_1_final_test_case_1/full_test_sleep.mat"
...
#LIST_EXP_TYPES=("case_3" "case_4")
LIST_EXP_TYPES=("case_3")

exp="NeurIPS_1_filter"
pretrain_best="True"

for SUB_EXP_TYPE in "${LIST_EXP_TYPES[@]}";
do
  task_exp="${exp}/${SUB_EXP_TYPE}"
  run_full_multi_gpu $gpu_device_1 $task_exp group_aug group_norm group_model group_datasets
  run_predict_task_1 $gpu_device_1 $task_exp $test_case_path group_aug group_norm group_model group_datasets

  tune_cla_exp="${task_exp}/${SUB_EXP_TYPE}/tune_cla"
  run_pretrain $gpu_device_1 $tune_cla_exp $task_exp $pretrain_best group_aug group_norm group_model group_datasets
  run_predict_task_1 $gpu_device_1 $tune_cla_exp $test_case_path group_aug group_norm group_model group_datasets

  tune_cla_exp="${task_exp}/${SUB_EXP_TYPE}/tune_cla_2"
  run_pretrain $gpu_device_1 $tune_cla_exp $task_exp $pretrain_best group_aug group_norm group_model group_datasets
  run_predict_task_1 $gpu_device_1 $tune_cla_exp $test_case_path group_aug group_norm group_model group_datasets

  tune_cla_exp="${task_exp}/${SUB_EXP_TYPE}/tune_full"
  run_pretrain $gpu_device_1 $tune_cla_exp $task_exp $pretrain_best group_aug group_norm group_model group_datasets
  run_predict_task_1 $gpu_device_1 $tune_cla_exp $test_case_path group_aug group_norm group_model group_datasets

  tune_cla_exp="${task_exp}/${SUB_EXP_TYPE}/tune_full_2"
  run_pretrain $gpu_device_1 $tune_cla_exp $task_exp $pretrain_best group_aug group_norm group_model group_datasets
  run_predict_task_1 $gpu_device_1 $tune_cla_exp $test_case_path group_aug group_norm group_model group_datasets
done
