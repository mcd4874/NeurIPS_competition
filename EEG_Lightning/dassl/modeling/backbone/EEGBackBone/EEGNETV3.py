...
class EEGNETV3(Backbone):
    def __init__(self, kern_legnth=64, num_ch=22, samples = 256,F1=8, D=2, F2=16,drop_prob=0.4,avg_pool_1 = 4, avg_pool_2 = 8, sep_kern_length = 16):
        ...
        print("calculate pad_h : {}, pad_w : {} .".format(pad_h,pad_w))
        print("expect pad_h : {}, pad_w : {} for odd ".format(0,sep_kern_length//2))
        if pad_h % 2 == 1 or pad_w % 2 == 1:
        ...
        samples = samples//(self.avg_pool_2*self.avg_pool_1)
...
def eegnet_v3(pretrained=False,pretrained_path = '', **kwargs):
    print("params set up : ",kwargs)
    model = EEGNETV3(**kwargs)
