...
class ShallowConvNet(Backbone):
    def __init__(self,F1 = 40, F2 = 40,kern_legnth = 31,num_ch = 64, samples = 128, drop_prob = 0.5,momentum = 0.1,eps = 1e-05,pooling_length = 35, pool_stride = 7):
        super().__init__()
        self.c1 = nn.Conv2d(in_channels=1, out_channels=F1, kernel_size=(1, kern_legnth), stride=1, bias=False)
        w1 = samples -(kern_legnth-1)
        self.c2 = nn.Conv2d(in_channels=F1, out_channels=F2, kernel_size=(num_ch, 1), stride=1, bias=False)
        w2 = w1
