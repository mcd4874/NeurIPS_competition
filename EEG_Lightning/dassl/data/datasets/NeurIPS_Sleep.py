...
class MultiDatasetV1(DatasetBase):
    ...
    def parse_config(self):
        ...
        self._target_dataset_names = self.cfg.DATAMANAGER.DATASET.SETUP.TARGET_DATASET_NAMES
    ...
    def _read_data(self,data_path,extra_data_path=None):
        ...
        for dataset in datasets:
            ...
            if dataset_name in self._target_dataset_names:
                print("add dataset {} into list target data ".format(dataset_name))
                list_target_data.extend(data)
                list_target_label.extend(label)
                list_target_meta_data.extend(meta_data)
            else:
                ...
                if len(self._source_dataset_names) > 0:
                    ...
                    if dataset_name in self._source_dataset_names:
                        list_source_data.extend(data)
                        list_source_label.extend(label)
                        list_source_meta_data.extend(meta_data)
                    else:
        ...
        if len(self._target_dataset_names) == 0:
            #there is no specific target data. We use 1 main dataset.
            target_data = list_source_data
            target_label = list_source_label
            target_meta_data = list_source_meta_data

            self.source_data_list = None
            self.source_label_list = None
            self.source_meta_data_list = None
        else:
            target_data = list_target_data
            target_label = list_target_label

            target_meta_data = list_target_meta_data
    ...
    def generate_test_data(self,data,label,subject_idx,current_test_fold,n_test_folds,shuffle_test_seed=-1,within_subject_split=True,train_valid_ratio=0.3):
        if within_subject_split:
            ...
            print("cross subject split for test data")
            train_valid_data,train_valid_label,train_valid_subject_idx, test_data, test_label,test_subject_idx= self.split_dataset_cross_subjects(data,label,subject_idx,current_fold=current_test_fold,n_folds=n_test_folds,split_ratio=train_valid_ratio,shuffle_trial_seed=shuffle_test_seed)
    ...
    def generate_valid_data(self,data,label,subject_idx,current_valid_fold,n_valid_folds,shuffle_valid_seed=-1,within_subject_split=True,train_ratio=0.8):
        if within_subject_split:
            ...
            print("cross subject split for valid data")
            train_data,train_label,train_subject_idx, valid_data, valid_label,valid_subject_idx= self.split_dataset_cross_subjects(data,label,subject_idx,current_fold=current_valid_fold,n_folds=n_valid_folds,split_ratio=train_ratio,shuffle_trial_seed=shuffle_valid_seed)
    ...
    def process_data_format(self,target_dataset):
        target_data, target_label, _ = target_dataset
        # unique_subject_id = np.unique(subject_meta['subject'])
        ...
        #not deal with increment for now
        ...
        use_target_subjects = available_subject_ids
        ...
        print("test subject idx after split : ",test_subject_idx)
        print("train-valid idx after split : ",train_valid_subject_idx)
        ...
        print("train subject after split : ",train_subject_idx)
        print("valid subject after split : ",valid_subject_idx)
    ...
    def split_dataset_cross_subjects(self,data,label,data_subject_ids,current_fold,n_folds,split_ratio=0.3,shuffle_trial_seed=-1):
        if split_ratio < 0.0:
            train_data, train_label,train_subject_ids, test_data, test_label,test_subject_ids = self.fold_split_cross_subject_data(data,label,n_folds=n_folds,current_fold=current_fold)
        else:
            train_data, train_label,train_subject_ids, test_data, test_label,test_subject_ids = self.shuffle_cross_subject_split_data(data,label,split_ratio=split_ratio,shuffle_trial_seed=shuffle_trial_seed)

        update_train_subjects = [data_subject_ids[i] for i in train_subject_ids]
        update_test_subjects = [data_subject_ids[i] for i in test_subject_ids]

        return train_data, train_label,update_train_subjects, test_data, test_label,update_test_subjects


    def shuffle_cross_subject_split_data(self,data,label,split_ratio=0.3,shuffle_trial_seed=-1):

        n_subjects = len(data)
        n_train_subjects = int(n_subjects*split_ratio)
        available_subject_ids = np.arange(n_subjects)

        if shuffle_trial_seed > -1:
            np.random.seed(shuffle_trial_seed)
            available_subject_ids = np.random.permutation(n_subjects)



        train_subjects = available_subject_ids[:n_train_subjects]
        test_subjects = available_subject_ids[n_train_subjects:]


        train_data = [data[subject_id] for subject_id in train_subjects]
        train_label = [label[subject_id] for subject_id in train_subjects]

        test_data = [data[subject_id] for subject_id in test_subjects]
        test_label = [label[subject_id] for subject_id in test_subjects]


        return train_data, train_label,train_subjects, test_data, test_label,test_subjects


    def fold_split_cross_subject_data(self, data, label, n_folds=4, current_fold=1):
        if current_fold > n_folds:
            raise ValueError("can not assign fold identity outside of total cv folds")

        n_subjects = len(data)
        available_subject_ids = np.arange(n_subjects)
        folds_subject = np.array_split(available_subject_ids,n_folds)

        test_subjects = folds_subject[current_fold-1]
        train_subjects = []
        for fold in range(len(folds_subject)):
            if fold != (current_fold-1):
                train_subjects.extend(folds_subject[fold])
