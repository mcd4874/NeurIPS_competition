...
class ZTransformNormalization:
    """Normalize EEG data using per-channel mean and standard deviation for single trial
    Z = (signal – baseline_ave)/ (n-1∑ (baseline_time – baseline_ave))
    assume the data format is (1,channels,times)
    ...
    def __call__(self, EEG_data):
        print("z transform is applied ")
        # print("current data shape : ",EEG_data.shape)
        axis = -1
        mean = EEG_data.mean(axis,keepdims=True)
        std = EEG_data.std(axis, keepdims=True)
        return (EEG_data-mean) / (std + self.eps)

class Fixscale:
    # def __init__(self, eps=1e-8):
    #     self.eps = eps

    def __call__(self, EEG_data,axis=-1):
        # print("apply fix scale")
        return EEG_data / np.max(np.abs(EEG_data))
...
def build_transform(cfg, is_train=True, choices=None):
    ...
    if is_train:
        return _build_transform_train(cfg, choices)
    else:
        return _build_transform_test(cfg, choices)


def _build_transform_train(cfg, choices):
    ...
    if 'z_transform' in choices:
        tfm_train += [ZTransformNormalization()]

    elif 'fixscale' in choices:
        tfm_train += [Fixscale()]
...
def _build_transform_test(cfg, choices,):
    ...
    if 'z_transform' in choices:
        tfm_test += [ZTransformNormalization()]
    elif 'fixscale' in choices:
        tfm_test += [Fixscale()]
