...
class CAN(TrainerX):
    """
    Modification of Conditional Adversarial Domain Adaptation for Generalization
    https://papers.nips.cc/paper/7436-conditional-adversarial-domain-adaptation
    """
    ...
    def build_data_loader(self):
        ...
        if self.cfg.DATALOADER.TRAIN_X.N_DOMAIN > self.dm.num_source_domains:
            self.n_domain = self.cfg.DATALOADER.TRAIN_X.N_DOMAIN
        else:
            self.n_domain = self.dm.num_source_domains
    ...
    def build_model(self):
        ...
        print('Building D')
        self.D = nn.Linear(d_fim, self.n_domain)
    ...
    def forward_backward(self, batch,backprob = True):
        ...
        for x, y, dy, d in zip(input_x, label_x, domain_x,d_x):
            ...
            #update conditional domain discriminator
            softmax_output = F.softmax(CC_logits,dim=1)
            feat_x = self.revgrad(feat)
            mix_feature = genenerate_mix_feature(feat_x,softmax_output)
            D_logits = self.D(mix_feature)
