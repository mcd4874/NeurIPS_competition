...
def save_checkpoint(
    state, save_dir, is_best=False, remove_module_from_keys=False,is_checkpoint = False
):
    r"""Save checkpoint.
    ...
    if is_checkpoint:
        fpath = osp.join(save_dir, 'model-checkpoint.pth.tar')
        print('latest checkpoint saved to "{}"'.format(fpath))
        ...
        print('Best checkpoint saved to "{}"'.format(fpath))
    else:
        epoch = state['epoch']
...
def resume_from_checkpoint(fdir, model, optimizer=None, scheduler=None):
    ...
    best_checkpoint_val = checkpoint['best_val_checkpoint']
    print("load Previous  val checkpoint: {} ".format(best_checkpoint_val))

    return start_epoch,best_checkpoint_val
