...
__all__ = [
    ...
    'check_availability', 'tolist_if_not','generate_path_for_multi_sub_model'
]
...
def generate_path_for_multi_sub_model(cfg,test_fold_prefix="",shuffle_fold_prefix="",increment_fold_prefix="",valid_fold_prefix=""):
    SHUFFLE_FOLD = cfg.DATAMANAGER.DATASET.SETUP.SHUFFLE_TRAIN_VALID_FOLD.N_SHUFFLE_FOLDS > 1
    INCREMENT_FOLD = cfg.DATAMANAGER.DATASET.SETUP.INCREMENT_FOLD.START_NUM_TRAIN_SUGJECT > 0
    # INCREMENT_FOLD = cfg.DATAMANAGER.DATASET.SETUP.INCREMENT_FOLD.INCREMENT_TRAIN_SUGJECT > 0
    # generate a folder for test k fold
    ...
    #     os.makedirs(path)
    if SHUFFLE_FOLD:
        path = osp.join(path, shuffle_fold_prefix)
        # if not osp.isdir(path):
        #     os.makedirs(path)
    # generate a folder for increment k fold
    if INCREMENT_FOLD:
