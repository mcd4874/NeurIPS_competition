...
class MultiShareMCD(TrainerMultiAdaptation):
    """
    ...
    def build_metrics(self):
        super(MultiShareMCD, self).build_metrics()
    ...
    def training_step(self, batch, batch_idx):
        ...
        for u, y, d in zip(list_input_u, list_label_u, domain_u):
            ...
            #     domain_weight = None
            source_batch = (u,y,d)
            loss, _, _, _ = self.share_step(source_batch, train_mode=True,weight=domain_weight)
            loss_source +=loss
        loss_source /= len(domain_u)

        # if self.no_target_weight:
        batch = self.parse_target_batch(target_batch)
        # loss_target, logit_1, logit_2, label = self.share_step(batch, train_mode=True)
        # else:
        ...
        #step B
        batch = self.parse_target_batch(target_batch)
        loss_x,_,_,_, = self.share_step(batch, train_mode=True)
        # loss_u = 0
        # for u, y, d in zip(list_input_u, list_label_u, domain_u):
        #     source_batch = (u,y,d)
        #     loss, _, _, _ = self.share_step(source_batch, train_mode=True)
        #     loss_u+=loss
        # loss_u /= len(domain_u)
        ...
        # loss_B = self.target_ratio*loss_x+self.source_ratio*loss_u-loss_dis
    ...
    def validation_step(self, batch, batch_idx, dataset_idx: Optional[int] = None):
        batch = self.parse_target_batch(batch)
