...
from NeurIPS_competition.util.support import (
    ...
    generate_data_file, filterBank
)
### generate data for FBCNet
...
X_src1 = np.moveaxis(filter_data, source, destination)
filter_data = filter(X_src2)
X_src2 = np.moveaxis(filter_data, source, destination)
filter_data = filter(X_src3)
X_src3 = np.moveaxis(filter_data, source, destination)
...
#use filter bank
filter_data = filter(X_MIA_train_data)
X_MIA_train_data = np.moveaxis(filter_data, source, destination)
filter_data = filter(X_MIB_train_data)
X_MIB_train_data = np.moveaxis(filter_data, source, destination)
...
def check_instance_type(data):
    print("check float32 instance : ",isinstance(data, np.float32))
    print("check float64 instance : ",isinstance(data, np.float64))
    print("check int32 instance : ",isinstance(data, np.int32))
    print("check int64 instance : ",isinstance(data, np.int64))
    print("check ndarray : ",isinstance(data, np.ndarray))


check_instance_type(X_src2.dtype)
check_instance_type(y_src2.dtype)
